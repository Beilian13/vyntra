...
async function joinLivekitRoom(lvRoomName, chatRoom){
  ...
  var room=new LivekitClient.Room({
    ...
    reconnectPolicy: { maxRetries: 10, retryDelayMs: 1000 },
    publishDefaults:{
      dtx:           true,   // discontinuous transmission — silence = no packets
      red:           true,   // redundant encoding — fixes packet loss artifacts
      simulcast:     false,
    ...
    audioCaptureDefaults:{
      ...
      sampleRate:         48000,
      channelCount:       1,
      // Use high-quality constraints for voice
  ...
  lk.room=room;lk.lvRoomName=lvRoomName;lk.active=true;
...
function setupVAD(rawStream){
  // VAD is now meter-only — Livekit handles noise suppression + DTX natively.
  // We just drive the visual meter and the vadStatus indicator.
  var AC=window.AudioContext||window.webkitAudioContext;
  try{ var ctx=new AC(); lk.audioCtx=ctx; } catch(e){ return; }
  ...
  var SPEAK_THRESH=0.015, HOLD_MS=300, lastSpeak=0;

  lk.vadInterval=setInterval(function(){
    if(!lk.active){ clearInterval(lk.vadInterval); return; }
    ...
    var vb=document.getElementById('vadBar');if(vb) vb.style.width=pct+'%';
    var vs=document.getElementById('vadStatus');if(vs) vs.textContent=speaking?'🟢':'🔇';
    // Speaking glow on local tile
